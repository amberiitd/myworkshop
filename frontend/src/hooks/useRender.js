...
const useRender = (canvasRef) => {
	...
	const renderImage = useCallback(async (file, options = {}) => {
		const { maxWidth = 168, maxHeight = 170 } = options;
		...
		image.onload = () => {
			...
			if (imgAspectRatio < 168 / 170) {
				canvas.height = Math.min(maxHeight, image.naturalHeight);
